...
| `versions/v3/PROJECT_GUIDE.md` + `.html` §1 yêu cầu 4: chú giải khung UART `GN 09\n` — liệt kê đủ 6 byte (`G`·`N`·space·`0`·`9`·LF `0x0A`) và ghi rõ `\n` là escape cho MỘT byte | LÀM RÕ TÀI LIỆU (không đổi nội dung kỹ thuật) | Rà soát tác giả 14/08 nghi `\n` là lỗi — kiểm chứng thực nghiệm (`printf 'GN 09\n' \| od`: `47 4e 20 30 39 0a` = đúng 6 byte) xác nhận đặc tả gốc ĐÚNG; biến thể mở rộng `S=GN T=09\n` = 10 byte cũng khớp. Chỉ thêm chú giải chống đọc nhầm "\n = 2 ký tự". |
| Rà soát nghi vấn "`<=` hiển thị thành ≤" — vòng 1 | KIỂM CHỨNG BYTE | Byte-level đúng chuẩn ở mọi nơi (ASCII `<`+`=` trong ngữ cảnh code; HTML dùng `&lt;=`); ≤ trong "mỗi pha ≤ 15 s"/"GREEN_TICKS ≤ 15" là so sánh toán học gốc — đúng. Kết luận ban đầu "chỉ là ligature của editor" **chưa đủ** — xem vòng 2. |
| **Vòng 2 (sau ảnh chụp của tác giả): tắt ligature vùng code TOÀN SITE** | SỬA HIỂN THỊ (không đổi byte nội dung) | Truy nguyên: `versions/v3/css/style.css` khai báo `--mono: ui-monospace,"Cascadia Code","JetBrains Mono",…` và V2 nạp webfont JetBrains Mono — các font này bật ligature mặc định nên trình duyệt VẼ chuỗi `<=` thành hình ≤ ngay trên site (đã tái hiện bằng Chromium + JetBrains Mono: ảnh before/after lưu hồ sơ phiên làm việc). Sửa: (a) thêm quy tắc `font-variant-ligatures: none; font-feature-settings: "liga" 0,"calt" 0,"dlig" 0` cho `code/pre/kbd/samp/.code-block/.mono` vào `src/styles/global.css`, `public/legacy/assets/css/site.css`, `versions/v3/css/style.css`; (b) chèn MỘT thẻ `<style id="icdh-no-code-ligatures">` tự chứa trước `</head>` của **23 trang tuần gốc V1/V2** bằng script `tools/patch-code-ligatures.mjs` (giữ trong repo, idempotent) — thuộc diện "sửa hiển thị, tối thiểu" của VERSIONING_POLICY, không đổi ký tự nội dung nào; (c) bọc backtick cặp toán tử trần trong bài "Ngân hàng câu hỏi thảo luận" (Tuần 7). Kiểm chứng sau sửa: cùng font ligature, chip code hiển thị đúng hai ký tự `<=`. |
