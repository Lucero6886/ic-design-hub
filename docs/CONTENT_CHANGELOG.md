...
| Rà soát nghi vấn "`<=` hiển thị thành ≤" | KHÔNG SỬA (đã kiểm chứng) | Kiểm tra byte-level: mọi chỗ viết toán tử non-blocking trong nội dung hub và bản vá đều là ASCII `<`+`=` trong ngữ cảnh code (HTML dùng `&lt;=`); site build hiển thị đúng `<=`. Các ký tự ≤ có thật trong PROJECT_GUIDE ("mỗi pha ≤ 15 s") và implementation-notes ("GREEN_TICKS ≤ 15") là **so sánh toán học** của văn bản gốc — dùng đúng. Hiện tượng thấy "≤" ở dòng style nhiều khả năng là font ligature của editor (Cascadia Code/Fira Code/JetBrains Mono vẽ `<=` thành hình ≤) — byte không đổi. |
